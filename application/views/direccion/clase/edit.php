                    ...
                    <tbody>
                        ...
                        foreach ($clase as $row) : ?>
                            <tr>
                                ...
                                <td>
                                    <div class="form-group">
                                        <select required="required" name="grupo_id[<?= $i ?>]" class="input-sm form-control select2" style="width: 100%!important">
                                            <option value="" disabled>-- Escoger --</option>
                                            <?php foreach ($grupo as $j) : ?>
                                                <option <?= $row->grupo_id == $j->id_grupo ? "selected='selected'" : "" ?> value="<?= $j->id_grupo ?>"><?= $j->nombre_grupo ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                        <small class="help-block text-right"></small>
                                    </div>
